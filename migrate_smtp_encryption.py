...
def migrate_smtp(username, password):
    ...
    try:
        with get_db_connection() as con:
            ...
            # --- Migrazione SMTP Password ---
            print("\nMigrazione SMTP Password...")
            cur.execute("SELECT valore FROM Configurazioni WHERE chiave = 'smtp_password' AND id_famiglia = %s", (id_famiglia,))
            row = cur.fetchone()
            
            if row:
                password_originale = row['valore']
                
                if password_originale and password_originale.startswith("gAAAA"):
                    print("SMTP Password sembra già criptata. Salto.")
                else:
                    encrypted_password = _encrypt_if_key(password_originale, family_key, crypto)
                    cur.execute("UPDATE Configurazioni SET valore = %s WHERE chiave = 'smtp_password' AND id_famiglia = %s", 
                                (encrypted_password, id_famiglia))
                    print("SMTP Password criptata con successo.")
            else:
                print("Nessuna configurazione SMTP password trovata per questa famiglia.")
